...
def on_press(root, event):
    ...
    root.direction = delta.get(event.keysym, None)

def on_release(root, event):
    root.direction = None

def animate(root,board):
    if root.direction is not None:
            update_drive(board)
    root.after(25, animate(root,board))

def update_drive(speed,board):
    pwm=int(speed)
    motor_driver(MOTOR_L, CW, pwm, board)
    motor_driver(MOTOR_R, CW, pwm, board)
...
if __name__ == '__main__':
    ...
    canvas.pack()
    canvas.bind("<Any-KeyPress>", on_press(root))
    canvas.bind("<Any-KeyRelease>", on_release(root))
    ...
    root.after(0, func=lambda: animate(root, board))
    root.mainloop()